...
def test_load_recording_from_trace_zip() -> None:
    trace_path = Path(__file__).resolve().parents[3] / "src" / "trace.zip"
    assert trace_path.exists(), "trace.zip test artifact must exist"

    manager = RecordingManager(trace_path=trace_path)
