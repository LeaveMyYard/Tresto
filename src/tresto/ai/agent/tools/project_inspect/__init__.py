...
async def _execute_file_exploration_cycle(
    ...
    while exploration_attempt < MAX_EXPLORATION_ATTEMPTS:
        ...
        if state.config.verbose:
            console.print(f"📁 Generating file exploration command (attempt {exploration_attempt}/{MAX_EXPLORATION_ATTEMPTS})...")
        else:
            console.print(f"📁 Generating exploration (attempt {exploration_attempt}/{MAX_EXPLORATION_ATTEMPTS})...")
        
        # Generate exploration command
        exploration_command = await generate_file_exploration_command(
            state, exploration_context, exploration_history
        )
        ...
        exploration_result = execute_file_exploration_command(
            actual_command, Path.cwd()
        )
